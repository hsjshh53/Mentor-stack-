...
import { 
  ...
  Layout, Database, Terminal, Calendar
} from 'lucide-react';
...
export const ProjectDetailsPage: React.FC = () => {
  ...
  const handleSetDueDate = async () => {
    if (!user || !project || !dueDateInput) return;
    
    const dueDate = new Date(dueDateInput).getTime();
    if (isNaN(dueDate)) {
      alert('Invalid date format');
      return;
    }

    await projectService.updateDueDate(user.uid, project.id, dueDate);
    
    setProgress(prev => {
      if (prev) return { ...prev, dueDate };
      // If no progress exists yet, we don't necessarily want to create it here 
      // unless we want to allow setting due dates before starting.
      // But for now, let's assume they've started or we handle it gracefully.
      return prev;
    });
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-white flex flex-col">
                        ...
                        <div className="p-4 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-0.5">
                              <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">Due Date</p>
                              <p className="text-sm font-bold">{new Date(progress.dueDate).toLocaleString()}</p>
                            </div>
                          </div>
                          <button 
                            onClick={() => {
                              setDueDateInput(new Date(progress.dueDate!).toISOString().slice(0, 16));
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setIsDueDateModalOpen(false)}>Cancel</Button>
      ...
      >
        <div className="space-y-6">
          <p className="text-sm text-white/40 leading-relaxed">
            Setting a due date helps you stay on track with your learning goals. We'll send you reminders as the date approaches.
          </p>
          <Input 
            label="Due Date & Time"
            type="datetime-local"
